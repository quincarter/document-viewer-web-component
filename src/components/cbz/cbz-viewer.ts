...
export class CbzViewer extends LitElement {
  ...
  private _initializeWorker() {
    try {
      this._cbzWorker = new Worker(
        new URL("./workers/cbz.worker.ts", import.meta.url),
        { type: "module" }
      );
  ...
  private _drawImageToCanvas(img: HTMLImageElement, isSecondPage = false) {
    ...
    if (this._shouldShowSinglePage(this._currentPage - 1)) {
      ...
      // Dual page mode - left page gets left half, right page gets right half
      ...
      // Calculate horizontal position based on which page this is
      const dx = isSecondPage
        ? halfWidth + (halfWidth - dWidth) / 2 // Right side
        : (halfWidth - dWidth) / 2; // Left side
