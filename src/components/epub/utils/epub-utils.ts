...
export class EpubManager {
	...
	async renderPage(pageNumber: number): Promise<string> {
		...
		let spineItem: SpineItem | null = null;
		let currentIndex = 0;
		...
		if (this.rendition) {
			await this.rendition.display((spineItem as SpineItem).href);
			return ""; // Content is handled by the rendition
		} else {
			const content = await (spineItem as SpineItem).load();
	...
	getPageFromHref(href: string): number {
		...
		let index = -1;
		this.book.spine.each((item: SpineItem, i: number) => {
			if (item.href === href) {
				index = i;
			}
		});
	...
	onRelocated(callback: (location: EpubLocation) => void): void {
		this.rendition?.on("relocated", callback);
	}

	removeRelocatedListener(callback: (location: EpubLocation) => void): void {
